...
def main():
    ...
    cyc = [float(r["cycles_per_pkt"]) for r in rows]

    fig, (a1, a2) = plt.subplots(1, 2, figsize=(14, 6))
    a1.bar(labels, mpps)
    a1.set_ylabel("throughput (Mpps)")
    a1.set_title("Throughput by configuration")
    a2.bar(labels, cyc)
    a2.set_ylabel("CPU cycles / packet")
    a2.set_title("Per-packet cost (lower is better)")
    for ax in (a1, a2):
